...
def Preprocessing(prot_id, querySeq, config_file):
	...
	global cache, cache_dir, work_dir, omaIdFile, orth_file, fas_file, aln_file, phy_file, id_file, proteome_file, tree_file, trans_file, hmm_file, xml_file, REvolver_output_dir, species_id, indel_file, scale_file
	...
	if prot_config.search_ortholog_groups:
		...
		print 'FAS score calculation needed. Skipping OMA group search.'
	...
	if prot_config.fas_score:
		if cache and os.path.exists(cache_dir + '/' + fas_file):
			...
			startProcessTime = time.time()
			calculateFAS(work_dir, prot_config.hamstr, prot_config.fas_annotations, os.path.abspath(orth_file), os.path.abspath(fas_file), os.path.abspath(domain_archi_file), prot_config.hamstr_oma_tree_map, prot_config.blastp, prot_id, species_id, delTemp)
...
def parseFAS(fasOutputFile,domainArchiFile,prot_id, sp1, sp2):
	name = prot_id
...
# FAS Calculation
def calculateFAS(working_dir, hamstr, speciesAnnoDir, orth_file, fas_file, domain_archi_file, map_file, blastp, prot_id, species_id, delTemp):
	...
	hamstr_blast_dir = hamstr + '/blast_dir_phytozome'
	...
	if not os.path.exists("%s_cacheData.txt" %prot_id):
		flog = open("%s_cacheData.txt" %prot_id, "w")
	else:
		flog = open("%s_cacheData.txt" %prot_id, "a")


	logDict = {}
	flog.write("### Orthologs identifiers ###\n")

	with open(orth_file) as of:
		for line in of:
			if '>' in line:
				omaId = line.split()[0][1:]
				oma_ids.append(omaId)
				sequence = of.next()

				for mapLine in open(map_file):
					if omaId == mapLine.split()[-1]:
				...
				ftemp = open("temp_query_fas.fa", "w")
				ftemp.write('>' + omaId + '\n' + sequence)
				ftemp.close()
				blast_command = '%s -query temp_query_fas.fa -db %s -evalue 0.00001 -outfmt 6 -max_target_seqs 1 -out temp_out_fas.txt' %(blastp, species_blast_dir)
				os.system(blast_command)
				if os.path.exists('temp_out_fas.txt') and len(open('temp_out_fas.txt').read().split('\n')) > 1:
					hit_id = open('temp_out_fas.txt').read().split('\n')[0].split('\t')[1]
					flog.write(omaId + '\t' + hamstrId + '\t' + hit_id + '\n')
					logDict[omaId] = omaId + ':' + hit_id

				os.system('rm -rf temp_out_fas.txt')
				os.system('rm -rf temp_query_fas.fa')
	...
	for elements in oma_ids:
		fas_score = "NA"
		if not elements == species_id:
			fas_command = 'python %s -s %s -p %s -r %s -o 1 -j temp_fas_score.xml' %(greedy_fas_script, temp_fasAnno + '/' + species_id, temp_fasAnno + '/' + elements, speciesAnnoDir + '/' + mapDict[species_id])				
			print 'FAS command: ', fas_command
			os.system(fas_command)

			# Parsing FAS output file
			print 'Parsing FACT score..'
			if os.path.exists('temp_fas_score.xml'):
				element = ET.parse('temp_fas_score.xml').getroot()
				#print element
				for atype in element.findall('set_protein'):
					fas_score = atype.get('score')

			
			fasFile.write(species_id + '\t' + logDict[species_id] + '\t' + elements + '\t' + logDict[elements] + '\t' + fas_score + '\n')
			
			print 'Extracting feature architecture for PhyloProfile..'
			parseFAS('temp_fas_score.xml', domain_archi_file, prot_id, species_id, elements)			

			#sys.exit()
			os.system('rm -rf temp_fas_score.xml')
...
def run_hamstrOneSeq(hamstr, orth_file, map_file, prot_id, formatdb, blastp, proteome, delTemp):
	...
	try:
		...
		taxaPath = hamstr + '/genome_dir_phytozome'
		...
		try:
			if not seqId == "NA" and delTemp:
				if os.path.exists('inputTaxaSet_oneSeq.txt'):
					command = 'perl %s -sequence_file=%s -seqid=%s -refSpec=%s -coreOrth=5 -minDist=species -maxDist=superkingdom -checkCoorthologsRef -cleanup -coreTaxa="inputTaxaSet_oneSeq.txt" -cleanup -fasoff -local -strict -rep -seqName=%s -addenv=phytozome' %(hamstrOneSeq, orth_file.split('/')[-1], seqId, refSpec, prot_id)
				else:
					command = 'perl %s -sequence_file=%s -seqid=%s -refSpec=%s -coreOrth=5 -minDist=species -maxDist=superkingdom -checkCoorthologsRef -cleanup -fasoff -local -strict -rep -seqName=%s -addenv=phytozome' %(hamstrOneSeq, orth_file.split('/')[-1], seqId, refSpec, prot_id)
				...
				if os.path.exists('inputTaxaSet_oneSeq.txt'):
					command = 'perl %s -sequence_file=%s -seqid=%s -refSpec=%s -coreOrth=5 -minDist=species -maxDist=superkingdom -checkCoorthologsRef -cleanup -coreTaxa="inputTaxaSet_oneSeq.txt" -fasoff -local -strict -rep -seqName=%s -addenv=phytozome' %(hamstrOneSeq, orth_file.split('/')[-1], seqId, refSpec, prot_id)
				else:
					command = 'perl %s -sequence_file=%s -seqid=%s -refSpec=%s -coreOrth=5 -minDist=species -maxDist=superkingdom -checkCoorthologsRef -fasoff -local -strict -rep -seqName=%s -addenv=phytozome' %(hamstrOneSeq, orth_file.split('/')[-1], seqId, refSpec, prot_id)

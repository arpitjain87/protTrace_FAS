			...
			try:
				...
				for j in range(len(hamstrMapFile) - 1):
					if tempSpecies == hamstrMapFile[j].split('\t')[1]:
						...
						break
				#print omaName
				matrixDict[omaName] += '#' + str(traceValue) 
				#print c, matrixDict[omaName]
				#c += 1
				#print 'yes'
...
def main(nexusTreeFile, mapFile, protId, spTree, plotFigTree, speciesMaxLikFile, species_id, cache_dir):
		...
		if line.split()[-1] == speciesId:
			...
			if os.path.exists(fasFile):
				for line2 in open(fasFile):
					if line2.split()[2] == line.split()[-1]:
						orth = line2.split()[3]
						fas = line2.split()[-1]
						break 		
			elif os.path.exists(orthFile):
				for line2 in open(orthFile):
					if '>' in line2 and line2.split()[0][1:] == line.split()[-1]:
						orth = "1"
						break

		matrixDict[line.split()[-1]] = orth + '#' + fas
	...
	matrixFile.write('geneID')
	for j in range(len(hamstrMapFile) - 1):
		ncbiId = "ncbi" + hamstrMapFile[j].split()[-2]
		matrixFile.write('\t' + ncbiId)
	matrixFile.write('\n' + protId)

	for j in range(len(hamstrMapFile) - 1):
		omaId = hamstrMapFile[j].split()[-1]
		matrixFile.write('\t' + matrixDict[omaId])		
